...
It is prodvided to illustrate how figures were prodcued and is not intended for reuse. It also contains several hardcoded paths.

p_dist_collect_gene.py is a Python 3 script for comparing p-value distributions for p-values associated with OCRs that are
near or not near certain genes. It takes four positional command-line arguments:
1) A list of genes of interest, one per line
2) A space-seperated file with each gene near each OCR, one pair per line, with the OCRs in column 4 and the genes in column 8
3) A space-seperated file with the p-value associated with each OCR (one per line), with OCRs in column 1 and p-values in column 4.
...
1) A space-seperated file with each gene near each OCR, one pair per line, with the OCRs in column 4 and the genes in column 8
2) A space-seperated file with the p-value associated with  each gene (one per line), with genes in column 1 and p-values in column 4.
